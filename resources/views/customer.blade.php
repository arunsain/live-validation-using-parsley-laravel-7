...
/* #first_name.parsley-error{
 ...
 .parsley-type, .parsley-required, .parsley-equalto, .parsley-pattern,.parsley-length,.parsley-custom-error-message,.parsley-checkemail{
  color:#ff0000;
 }
...
$(document).ready(function(){  
 ...
 $('#validate_form').on('submit', function(event){
  ...
  if($('#validate_form').parsley().whenValidate())
  {
   ...
   $.ajax({
    ...
    {
      if(data.success){
        swal("Thats good!", "You save successfully!", "success");

      }else{
        swal("Ohh!", "Some think went Wrong", "error");
